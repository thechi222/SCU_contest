...
def test_seed_creates_demo_accounts_once():
    ...
    User.objects.filter(student_id="DEMO0001").update(name="改過的名字")
    call_command("seed", stdout=io.StringIO())

    assert User.objects.count() == 2
    assert User.objects.get(student_id="DEMO0001").name == "改過的名字"
...
def test_import_users_creates_accounts_with_random_passwords(tmp_path):
    ...
    source.write_text(
        "student_id,name,role\n11172001,受測者一,student\nstaff-0012,受測者二,staff\n",
        encoding="utf-8",
    )
    ...
    assert [row["student_id"] for row in rows] == ["11172001", "STAFF0012"]
    assert rows[0]["password"] != rows[1]["password"]
    assert User.objects.get(student_id="11172001").check_password(rows[0]["password"])


@pytest.mark.django_db
def test_import_users_rejects_invalid_role(tmp_path):
    source = tmp_path / "accounts.csv"
    source.write_text("student_id,name,role\n11172002,訪客,external\n", encoding="utf-8")
    output = tmp_path / "credentials.csv"
