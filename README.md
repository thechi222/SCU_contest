...
這是固定任務(§4.5)採任務型設計的主要理由:機主隨時可以收回設備,未完成的檔案自動由其他設備接手,
且全程不需要任何人連進機台。

互動式租借(§4.10)是唯一的例外:使用者必須真的連進容器,因此需要一條由**機台端主動建立**的對外通道。
平台仍然不會連向機台,但機台要自行開通道,這也是該功能的通道方案尚未定案的原因
(候選方案見 [架構文件](docs/architecture.md))。
...
2. 機主於 /nodes/ 開啟分享,機台 agent enable
     ↓
3. 於 /dashboard/ 確認設備狀態為「閒置可用」
     ↓
4. 上傳一批音訊與圖片
     ↓
5. 大螢幕與儀表板可見工作分別派給不同設備,GPU 使用率上升
     ↓
6. 關閉其中一台的分享 → 該檔案回到排隊,由另一台接手完成
     ↓
7. 下載逐字稿、字幕與放大後的圖片
     ↓
...
第 9 步須待機台端的租借容器與連線通道完成(§4.10);在此之前只驗收到申請進入佇列為止。
