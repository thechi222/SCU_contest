...
class Command(BaseCommand):
    help = (
        "Create accounts from a CSV with columns email,name,role. "
    ...
    def handle(self, *args, csv_path, output, **options):
        ...
        with open(csv_path, newline="", encoding="utf-8-sig") as f:
            for line_no, row in enumerate(csv.DictReader(f), start=2):
                ...
                user = User(username=email, email=email, name=row["name"].strip(), role=row["role"].strip())
                user.set_password(password)
                try:
                    user.full_clean()
                except ValidationError as exc:
                    raise CommandError(f"line {line_no} ({email}): {exc.messages}") from exc
                users.append(user)
